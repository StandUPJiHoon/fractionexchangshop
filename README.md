...
<html lang="ko">
 ...
 <body>
   ...
   <div class="container">
    <div class="header">
     <div class="header-top">
      <h1 id="app-title">🏪 분수 교환 상점</h1><button class="header-toggle-btn" id="toggle-shop-btn">👁️ 분수 상점 숨기기</button>
     </div>
     <p id="description-1">대분수를 변환하여, 분수끼리 뺄 수 없는 대분수 뺄셈을 연습해요.</p>
     <p id="description-2">대분수를 분수 교환 상점에 끌어 놓으면, 다른 분수로 변환되는 모습을 비교할 수 있어요.</p>
    </div>
    <div class="section" style="margin-bottom: 32px;">
     <h2 class="section-title">문제</h2>
     <div class="problem-display">
      <p class="problem-text" id="problem-text"><span class="mixed-number">2<span class="fraction"><span class="fraction-numerator">1</span><span class="fraction-denominator">4</span></span></span> − <span class="mixed-number">1<span class="fraction"><span class="fraction-numerator">3</span><span class="fraction-denominator">4</span></span></span></p>
     </div>
     ...
     </div><button class="new-problem-btn" id="new-problem-btn">🔄 새 문제 만들기</button>
    </div>
    <div class="shops-container" id="shops-container">
     <div class="shop-section shop1">
      <h2 class="shop-title" id="shop1-title">🟢 상점 1</h2>
     ...
     <div class="shop-section shop2">
      <h2 class="shop-title" id="shop2-title">🟢 상점 2</h2>
    ...
    <div class="main-content" id="main-content">
     <div class="section">
      <h2 class="section-title">🟢 상점 1 이미지 표현</h2>
      <div class="visualization-area" id="visualization-area-1">
       <div class="vis-content">
        ...
        <div id="no-exchange-message-1">
         <p style="color: #a0aec0; font-size: 16px; margin-top: 32px;">상점 1에 아직 넣지 않았어요.</p>
        </div>
       </div>
      </div>
     </div>
     <div class="section">
      <h2 class="section-title">🟢 상점 2 이미지 표현</h2>
      <div class="visualization-area" id="visualization-area-2">
       <div class="vis-content">
        ...
        <div id="no-exchange-message-2">
         <p style="color: #a0aec0; font-size: 16px; margin-top: 32px;">상점2에 아직 넣지 않았어요.</p>
        </div>
       </div>
      </div>
     </div>
    ...
    <div class="answer-section">
     <div class="answer-prompt">
      이제 대분수끼리의 뺄셈을 계산해 보고, 정답을 대분수로 써 보세요.
     </div>
     ...
     <div class="action-buttons"><button class="btn btn-primary" id="check-answer-btn">✓ 정답 확인</button> <button class="btn btn-secondary" id="show-solution-btn">📖 해설보기</button>
     </div>
  ...
  <script>
    const defaultConfig = {
      app_title: "🏪 분수 교환 상점",
      description_1: "대분수를 변환하여, 분수끼리 뺄 수 없는 대분수 뺄셈을 연습해요.",
      description_2: "대분수를 분수 교환 상점에 끌어 놓으면, 다른 분수로 변환되는 모습을 비교할 수 있어요.",
      shop1_title: "🟢 상점 1",
      shop2_title: "🟢 상점 2",
      new_problem_button: "🔄 새 문제 만들기",
      toggle_shop_button: "👁️ 분수 상점 숨기기",
      check_answer_button: "✓ 정답 확인",
      show_solution_button: "📖 해설보기",
    ...
    function updateProblemDisplay() {
      ...
      document.getElementById('problem-text').innerHTML = 
        `<span class="mixed-number">${leftWhole}<span class="fraction"><span class="fraction-numerator">${leftNum}</span><span class="fraction-denominator">${denom}</span></span></span> − <span class="mixed-number">${rightWhole}<span class="fraction"><span class="fraction-numerator">${rightNum}</span><span class="fraction-denominator">${denom}</span></span></span>`;
    ...
    function toggleShops() {
      ...
      if (shopsVisible) {
        ...
        toggleBtn.textContent = '👁️ 분수 상점 숨기기';
      } else {
        shopsContainer.classList.add('hidden');
        mainContent.classList.add('hidden');
        toggleBtn.classList.add('active');
        toggleBtn.textContent = '👁️ 분수 상점 보이기';
      }
    }
    ...
    function handleExchange(side, shopType) {
      ...
      if (shopType === 1) {
        if (newWhole === 0) {
          textDiv.innerHTML = `<span class="mixed-number">${whole}<span class="fraction"><span class="fraction-numerator">${num}</span><span class="fraction-denominator">${denom}</span></span></span> → <span class="fraction"><span class="fraction-numerator">${newNum}</span><span class="fraction-denominator">${denom}</span></span>`;
          stepsDiv.innerHTML = `자연수 1을 <span class="fraction"><span class="fraction-numerator">${denom}</span><span class="fraction-denominator">${denom}</span></span>로 바꾸고 더했어요`;
        } else {
          textDiv.innerHTML = `<span class="mixed-number">${whole}<span class="fraction"><span class="fraction-numerator">${num}</span><span class="fraction-denominator">${denom}</span></span></span> → <span class="mixed-number">${newWhole}<span class="fraction"><span class="fraction-numerator">${newNum}</span><span class="fraction-denominator">${denom}</span></span></span>`;
          stepsDiv.innerHTML = `자연수 부분 중 1을 <span class="fraction"><span class="fraction-numerator">${denom}</span><span class="fraction-denominator">${denom}</span></span>로 바꾸고 더했어요`;
        }
      } else {
        textDiv.innerHTML = `<span class="mixed-number">${whole}<span class="fraction"><span class="fraction-numerator">${num}</span><span class="fraction-denominator">${denom}</span></span></span> → <span class="fraction"><span class="fraction-numerator">${newNum}</span><span class="fraction-denominator">${denom}</span></span>`;
        stepsDiv.innerHTML = `모든 자연수를 분수로: ${whole} × ${denom} + ${num} = ${newNum}`;
      }
    ...
    function updateVisualizationForExchange(side, originalWhole, originalNum, newWhole, newNum, denom, shopType) {
      ...
      originalLabel.textContent = `원래: ${originalWhole} ${originalNum}/${denom}`;
      ...
      if (shopType === 1) {
        ...
        if (newWhole === 0) {
          newLabel.textContent = `교환 후: ${newNum}/${denom}`;
        } else {
          newLabel.textContent = `교환 후: ${newWhole} ${newNum}/${denom}`;
        }
      ...
      if (shopType === 1) {
        ...
        conversionText.innerHTML = `<span class="fraction"><span class="fraction-numerator">1</span><span class="fraction-denominator">${denom}</span></span>이 ${totalNum}개`;
      }
      
      const conversionSteps = document.getElementById(`conversion-steps-${shopType}`);
      if (shopType === 1) {
        conversionSteps.innerHTML = `자연수 부분 중 1을 <span class="fraction"><span class="fraction-numerator">${denom}</span><span class="fraction-denominator">${denom}</span></span>로 바꿔서 더했어요`;
      } else {
    ...
    function checkAnswer() {
      ...
      if (isNaN(inputNum) || isNaN(inputDenom)) {
        ...
        feedback.textContent = '분자와 분모에 숫자를 입력해주세요.';
        return;
      }
      ...
      if (inputDenom !== denom) {
        feedback.className = 'feedback incorrect';
        feedback.style.display = 'block';
        feedback.textContent = '분모는 문제에 나온 분모와 같아야 해요.';
        return;
      }
      ...
      if (inputWhole === answerWhole && inputNum === answerNum) {
        ...
        feedback.style.display = 'block';
        feedback.textContent = '정답입니다! 대분수를 가분수로 잘 바꾸고, 정확하게 계산했어요!';
      } else if (inputWhole === answerWhole) {
        feedback.className = 'feedback incorrect';
        feedback.style.display = 'block';
        feedback.textContent = '자연수 부분은 맞지만 분자를 다시 계산해 볼까요?';
      } else {
        feedback.className = 'feedback incorrect';
        feedback.style.display = 'block';
        feedback.textContent = '대분수를 가분수로 바꾸는 과정부터 다시 한 번 차근히 살펴보세요.';
      }
    }
    ...
    function showSolution() {
      ...
      steps.innerHTML = `
        <div class="solution-step"><span class="mixed-number">${leftWhole}<span class="fraction"><span class="fraction-numerator">${leftNum}</span><span class="fraction-denominator">${denom}</span></span></span> − <span class="mixed-number">${rightWhole}<span class="fraction"><span class="fraction-numerator">${rightNum}</span><span class="fraction-denominator">${denom}</span></span></span></div>
        ...
        <div class="solution-step"><span class="fraction"><span class="fraction-numerator">${leftImproper}</span><span class="fraction-denominator">${denom}</span></span> − <span class="fraction"><span class="fraction-numerator">${rightImproper}</span><span class="fraction-denominator">${denom}</span></span> = <span class="fraction"><span class="fraction-numerator">${resultImproper}</span><span class="fraction-denominator">${denom}</span></span></div>
        ${lastStepText}
        <div class="solution-step">따라서 답은 ${answerDisplay}입니다.</div>
      `;
    ...
    async function onConfigChange(config) {
      ...
      if (shopsVisible) {
        ...
        toggleBtn.textContent = baseText.replace('숨기기', '보이기');
      }
